...
# yum update
RUN yum -y --enablerepo=remi,remi-php56 update --nogpgcheck; yum clean all
...
# Add Magento Config
ADD nginx.conf.sample.mage2.1.7 /etc/nginx/mage.nginx.conf
...
##Worked well
#RUN yum -y --enablerepo=remi,remi-php56 install php php-fpm php-common
...
RUN yum -y --enablerepo=remi,remi-php56 install php-imagick php-intl php-curl php-xsl php-mcrypt php-mbstring php-bcmath php-gd php-zip
...
#RUN yum -y --enablerepo=remi,remi-php56 install python-setuptools
RUN yum -y --enablerepo=remi,remi-php56 install supervisor
